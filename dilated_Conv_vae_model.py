...
class Encoder(nn.Module):
    ...
    def __call__(self, x):
        ...
        #0
        x = nn.Conv(512, kernel_size=(3,3),  strides=[1,1], kernel_dilation=1, padding='same')(x)
        x = nn.relu(x)
        ...
        #2
        x = nn.Conv(256,kernel_size=(3,3), strides=[1,1], kernel_dilation=2, padding='same')(x)
        x = nn.relu(x)
        x = nn.normalization.BatchNorm(True)(x)
 
        #3
        x = nn.Conv(128,kernel_size=(3,3), strides=[1,1], kernel_dilation=2, padding='same')(x)
        x = nn.relu(x)
        x = nn.normalization.BatchNorm(True)(x)
        
        #4
        x = nn.Conv(64, kernel_size=(3,3), strides=[1,1], kernel_dilation=4, padding='same')(x)
        x = nn.relu(x)
        x = nn.normalization.BatchNorm(True)(x)
        x = nn.max_pool(x, window_shape=(2,2), strides=(2,2))

        #5
        x = nn.Conv(32, kernel_size=(3,3), strides=[1,1], kernel_dilation=4, padding='same')(x)
        x = nn.relu(x)
        x = nn.normalization.BatchNorm(True)(x)
        
        #6
        x = nn.Conv(16, kernel_size=(3,3), strides=[1,1], kernel_dilation=4, padding='same')(x)
        x = nn.relu(x)
        x = nn.normalization.BatchNorm(True)(x)
...
class Dilated_Conv2d_VAE(nn.Module):
