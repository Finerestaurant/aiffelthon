...
def linear_freeze_train_step(enc_state, 
    ...
    return linear_state.apply_gradients(grads=grads), loss, accuracy, top_5_accuracy
...
def linear_unfreeze_train_step(state, x, y):    
    ...
    accuracy = jnp.mean(jnp.argmax(logits, -1) == jnp.argmax(y, -1))
    top_5_accuracy = top_k(logits, y, 5)
    return state.apply_gradients(grads=grads), loss, accuracy, top_5_accuracy
...
#             test_loss_mean += test_loss
            ...
            print(f'step : {j}/{len(train_dataloader)}', end='\r')
    ...
    # freeze_state = cloudpickle.dumps(state)
...
#             linear_test_top_5_accuarcy = 0
            ...
            for j in tqdm(range(len(train_dataloader))):
                ...
                test_x = (test_x / 200) + 0.5
                if config['freeze_encoder']:
                    linear_state, train_loss, train_accuarcy, train_top_5_accuarcy = linear_freeze_train_step(enc_state, enc_batch, linear_state, x, y)
                    test_loss, test_accuarcy, test_top_5_accuarcy = linear_freeze_eval_step(enc_state, enc_batch, linear_state, test_x, test_y)
                
                else:
                    state, train_loss, train_accuarcy, train_top_5_accuarcy = linear_unfreeze_train_step(state, x, y)
...
#                 linear_test_top_5_accuarcy += test_top_5_accuarcy
        ...
        # unfreeze_state = cloudpickle.dumps(state)
wandb.finish()
